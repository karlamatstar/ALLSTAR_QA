...
def test_judge_cases_loadable():
    """단일 test_cases.json에서 시연용 대표 3건을 읽을 수 있는가."""
    cases = load_json("test_cases.json")["cases"]
    assert len(cases) == 3
    assert [case["case_id"] for case in cases] == ["TC-01", "TC-04", "TC-09"]
    for c in cases:
    ...
    assert sum(case["judge_enabled"] for case in cases) == 3


def test_case_archives_and_active_sources_are_preserved():
    """20건·10건 아카이브와 현재 대표 3건의 출처 연결이 보존되는가."""
    active = load_json("test_cases.json")["cases"]
    archive_10 = load_json("archive/test_cases_10_2026-07-25.json")["cases"]
    archive_20 = load_json("archive/test_cases_20_2026-07-17.json")["cases"]

    assert len(archive_20) == 20
    assert len(archive_10) == 10
    assert [case["archive_source_case_id"] for case in archive_10] == [
        f"TC-{number:02d}" for number in range(1, 20, 2)
    ]
    assert [case["case_id"] for case in active] == ["TC-01", "TC-04", "TC-09"]
    assert [case["archive_source_case_id"] for case in active] == ["TC-01", "TC-07", "TC-17"]
