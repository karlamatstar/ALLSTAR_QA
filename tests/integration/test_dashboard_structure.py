...
def test_top_navigation_has_four_left_and_three_right_tabs():
    ...
    assert 'default="모니터링\\n(Monitoring)"' in APP
    assert 'key="allstar_top_navigation"' in APP
...
def test_report_collection_has_six_child_tabs_and_profile_comparison():
    ...
    for label in (
        ...
        assert label in VIEWS
    report_renderer = VIEWS.split("def _render_markdown_report", 1)[1].split("def _voc_report_signature", 1)[0]
    assert 'st.download_button(' in report_renderer
    assert '"보고서 다운로드"' in report_renderer
    assert 'disabled=not exists' in report_renderer
    assert 'key=f"report_download_{safe_key}"' in report_renderer
    assert 'width="content"' in report_renderer
    assert 'st-key-report_download_' in APP
    assert '--allstar-download-bg:#e7f7ec' in APP
    assert 'background:var(--allstar-download-bg) !important' in APP
