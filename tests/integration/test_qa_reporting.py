...
def test_pytest_summary_does_not_count_digits_in_test_name():
    output = """
tests/test_password.py::test_password_defaults_to_1234 PASSED [ 30%]
=========== 299 passed, 2 skipped, 2 deselected in 8.13s ============
"""

    assert qa_reporting.parse_pytest_summary(output) == {
        "passed": 299,
        "skipped": 2,
        "deselected": 2,
    }
...
def test_voc_profile_runner_can_limit_agent_validation_to_two_cases(tmp_path):
    ...
    command = runner.build_judge_command(tmp_path / "reports", ["TC-01", "TC-04"])

    assert command.count("--case-id") == 2
    assert "TC-01" in command
    assert "TC-04" in command
...
def test_partial_voc_validation_is_not_full_formal_report_scope():
    ...
    assert runner.is_full_case_scope(["TC-01", "TC-04"]) is False
