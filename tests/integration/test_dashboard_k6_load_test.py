...
def test_k6_runner_image_contains_runtime_regression_assets() -> None:
    dockerfile = (
        Path(__file__).resolve().parents[2]
        / "ops"
        / "docker"
        / "Dockerfile.k6_runner"
    ).read_text(encoding="utf-8")

    assert "compose.aws.yml" in dockerfile
    assert ".env.example" in dockerfile
