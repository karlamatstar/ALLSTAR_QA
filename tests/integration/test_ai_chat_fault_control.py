...
def test_service_control_bridge_sends_internal_token(monkeypatch):
    captured = {}
    monkeypatch.setattr(control, "SERVICE_CONTROL_URL", "http://service-control:8300")
    monkeypatch.setattr(control, "SERVICE_CONTROL_TOKEN", "bridge-secret")
    ...
    changed, detail = control._change_service_state("portfolio-api", "start")

    assert changed is True
    assert detail == "컨테이너 상태: running"
    assert captured == {
        "url": "http://service-control:8300/services/portfolio-api/start",
        "headers": {"X-AllStar-Service-Token": "bridge-secret"},
    }
