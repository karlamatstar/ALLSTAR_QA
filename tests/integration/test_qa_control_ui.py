...
def test_ai_testcase_quality_test_is_last_ai_tab_and_uses_all_cases():
    ...
    assert qa_control.load_ai_case_count() == 3
...
def test_validation_command_excludes_external_ai_tests_by_default():
    ...
    assert "# 인프라 장애·기능 검증 결과 보고서" in source
    assert "[장애·기능 검증] /fault-lab API 장애 재현 및 기능 검증 결과" in source
    assert "대시보드의 `K6 부하 테스트`에서 `장애·기능 검증 시험` 실행" in source
    assert "GUI '장애·기능 검증 시험' 실행" not in source
