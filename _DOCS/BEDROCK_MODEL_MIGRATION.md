...
| 기능 | 모델 | AWS 경로 | 리전 |
|---|---|---|---|
| 일반 교육과정 챗봇 답변 | gpt-oss-20b | Bedrock Mantle Responses API | `us-west-2` |
| 일반 챗봇 독립 채점 | gpt-oss-120b | Bedrock Mantle Responses API | `us-west-2` |
| VOC OpenAI 계열 생성 | gpt-oss-20b | Bedrock Mantle Responses API | `us-west-2` |
| VOC OpenAI 계열 Judge | gpt-oss-120b | Bedrock Mantle Responses API | `us-west-2` |
...
GPT-5.6 Luna·Terra와 GPT-5.4는 모델 목록에는 표시되지만 현재 AWS 계정의 실제 호출 권한에는 포함되지 않는다. 장기 Bedrock API 키를 만들거나 외부 제공자 키를 다시 저장하지 않고 EC2 IAM 역할만 사용하는 운영 원칙을 유지하기 위해, 생성에는 `openai.gpt-oss-20b`, 독립 평가에는 더 큰 `openai.gpt-oss-120b`를 사용한다.

## 성능 테스트 적용

`ops/performance/api_latency_test.js`는 실제 `/chat`을 호출하므로 gpt-oss-20b의 Bedrock Mantle 왕복시간과 애플리케이션 처리시간을 함께 측정한다. 1명·10명·25명 단계는 각각 독립 실행되며 결과 태그에 `amazon-bedrock`과 모델 ID를 남긴다.
...
실제 모델 호출은 AWS 권한과 과금이 발생하므로 승인된 AWS 환경에서 대표 입력만 제한적으로 검증한다.
