...
from allstar.ai_agent.api.config import AI_JUDGE_MODEL
        ...
        try:
            with openai_call_semaphore:
                response = _client().generate(
                    prompt,
                    max_tokens=int(os.getenv("AI_JUDGE_MAX_OUTPUT_TOKENS", "2200")),
                    reasoning=os.getenv("AI_JUDGE_REASONING", "low"),
                )
            break
