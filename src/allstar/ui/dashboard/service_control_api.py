...
SERVICE_CONTROL_TOKEN = os.getenv("SERVICE_CONTROL_TOKEN", "").strip()
...
def _require_internal_token(token: str | None) -> None:
    """내부 호출 토큰을 검증하되 토큰 값은 오류나 로그에 남기지 않는다."""
    if not SERVICE_CONTROL_TOKEN:
        raise HTTPException(status_code=503, detail="서비스 제어 내부 토큰이 설정되지 않았습니다.")
    supplied = (token or "").strip()
    if not supplied or not hmac.compare_digest(supplied, SERVICE_CONTROL_TOKEN):
        raise HTTPException(status_code=401, detail="서비스 제어 내부 인증에 실패했습니다.")
...
def get_service(
    service_name: str,
    x_allstar_service_token: str | None = Header(default=None),
) -> dict[str, Any]:
    _require_internal_token(x_allstar_service_token)
    try:
