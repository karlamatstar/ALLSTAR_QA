...
def _render_markdown_report(title: str, path: Path, description: str) -> None:
    exists = path.exists()
    safe_title = re.sub(r"[^0-9A-Za-z가-힣_-]+", "_", title).strip("_") or path.stem
    safe_key = re.sub(r"[^0-9A-Za-z_-]+", "_", path.as_posix()).strip("_")
    st.download_button(
        "보고서 다운로드",
        data=path.read_bytes() if exists else b"",
        file_name=f"{safe_title}{path.suffix or '.md'}",
        mime="text/markdown; charset=utf-8",
        disabled=not exists,
        key=f"report_download_{safe_key}",
        width="content",
    )
...
def render_reports() -> None:
    ...
    for index, (path, description) in zip((0, 1, 2, 4, 5), paths):
        with tabs[index]:
            _render_markdown_report(report_labels[index], path, description)
    with tabs[3]:
