...
def _active_case_scope_text() -> tuple[str, str]:
    cases = json.loads(TEST_CASE_PATH.read_text(encoding="utf-8"))["cases"]
    scored = [
        case["case_id"]
        for case in cases
        if case.get("judge_enabled") and not case.get("expect_no_data")
    ]
    exceptions = [case["case_id"] for case in cases if case.get("expect_no_data")]
    timed = [case["case_id"] for case in cases if case.get("judge_mode") == "live"]

    score_parts = []
    if scored:
        score_parts.append(f"정식 점수는 {', '.join(scored)}")
    if exceptions:
        score_parts.append(f"데이터 없음 예외처리는 {', '.join(exceptions)}")
    summary = ", ".join(score_parts) + "에서 확인"
    timing = f"평균점수는 {', '.join(scored)}, 수행시간은 {', '.join(timed)} 실제 실행 사례의 중앙값"
...
def slide_summary(prs: Presentation, results: list[Experiment]):
    ...
    scope_summary, _ = _active_case_scope_text()
...
def slide_charts(prs: Presentation, results: list[Experiment]):
    ...
    slide.background.fill.solid()
    slide.background.fill.fore_color.rgb = PALE
    _, timing_summary = _active_case_scope_text()
    add_header(slide, "02", "품질과 속도 비교", timing_summary)
