...
def create_chaos_defect_report(k6_out, pytest_out):
    ...
    content = f"""# 인프라 장애·기능 검증 결과 보고서

**작성일시**: {timestamp}
**테스트 범위**: `/fault-lab` API 장애 재현(Delay, 500 오류, Timeout 등) 및 Pytest 기능 통합 검증

## 결함 제목
[장애·기능 검증] /fault-lab API 장애 재현 및 기능 검증 결과

## 재현 절차
대시보드의 `K6 부하 테스트`에서 `장애·기능 검증 시험` 실행 (k6 `chaos_test.js` 호출) 및 기능 검증(Pytest) 자동화 파이프라인 수행
...
def convert_md_to_word(md_file: Path):
    ...
    doc = Document()
    doc.add_heading("인프라 장애·기능 검증 결과 보고서", 0)
...
조치 완료 후 재시험(Re-test)하여 본 티켓을 Closed 처리해 주시기 바랍니다."""
    ...
    payload = {
        "fields": {
            ...
            "issuetype": {"name": "Bug"},
            "priority": {"name": "High"},
            "summary": "[장애·기능 검증] /fault-lab API 장애 재현 및 기능 검증 결과",
